...
2. **Test-plan** - invoke test-plan. Decline its file-write offer; scenarios stay inline. At its end, append settled scenarios as `## Test scenarios`.
3. **Launch gate** - print the summary and nothing else: proposed outcome (1-2 sentences drafted from the decisions file), decisions one-liners, remaining stages, git posture (branch if on main; commits flow through the user's `COMMIT_AUTHORISED` gate - land only while open, else deferred), abort conditions, expected artifacts. Then WAIT. The user's response is confined to: confirm the outcome, correct it, or replace it - and `go` (with or without an outcome correction) is the sole entry to the autonomous zone. No flag skips the gate and no stage-skip options exist or may be offered. Only after `go`: write the confirmed outcome (proposal as-is, or the user's correction) to the decisions file as `## Outcome`, then proceed. Every invocation resuming into cases 3-7 passes the gate; case 8 and interview restarts do not.
4. **Spec** - run write-spec. Its user-approval gate is REPLACED: the reviewer verdict stands in for user approval. Scenarios from stage 2 become the spec's acceptance-criteria section. Then review (below).
5. **Plan** - run writing-plans; plan file named `<date>-<topic>-plan.md` (shared slug). Its approval gate REPLACED likewise. Then review.
6. **Implement** - spawn fresh general-purpose subagent (never a fork): plan path, spec path, decisions path, instructions to follow executing-plans and invoke the tdd skill. Prompt carries the implementer contract (below). If on default branch, first `git checkout -b hayden/<topic>`. On implement completion, append `Implemented` to the plan footer.
...
You execute the plan file. You NEVER ask the user questions, never push, and never set the commit authorization variable yourself. Plan "Commit" steps run as logical per-task commits through the user's env gate, as ONE chained command: `git add <files> && printenv COMMIT_AUTHORISED && git commit -m "<message>"`. Gate open → commit lands. Gate closed → chain fails → record "commit deferred at task N - gate closed" in your deviation log and CONTINUE (never run a bare commit). When all tasks complete, append `Implemented` to the plan footer. Blocked comes in three tiers:
