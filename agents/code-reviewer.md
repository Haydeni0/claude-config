...
1. **Identify what changed**
   ...
   - Otherwise:
     ...
     ```
     Do NOT use `git merge-base` here — it returns a commit older than the true base when the repo uses squash or rebase merges, inflating the diff with already-merged changes.

   Then:
   - `git diff $BASE..HEAD --stat` — check diff size
   ...
   - `git diff $BASE..HEAD` (with any scope filter) — full diff
   - `git diff` and `git diff --cached` — catch uncommitted changes on top
...
4. **Dispatch meta-reviewer**
   - Use the `Agent` tool with `subagent_type: meta-reviewer`. Pass review v1 (full markdown) and `$BASE` as the base ref in the prompt (same ref used for the diff above).
